...
impl GhStub {
    ...
    /// Make `gh run list` never return a run, simulating a dispatch that
    /// never registered — usually a ref whose workflow file lacks the trigger.
    pub fn run_never_appears(mut self) -> Self {
        self.env.insert("STUB_RUN_FOUND".into(), "0".into());
        self
    }

    /// A run of the same workflow on the same ref, triggered by a push
    /// rather than by gh-ship. Creating the staging branch is a push, so a
    /// prepare workflow declaring `on: push` produces exactly this.
    pub fn push_run(mut self) -> Self {
        self.env.insert("STUB_PUSH_RUN".into(), "1".into());
        self
    }

    /// A workflow generated by an older gh-ship: `ship_id` is a required
    /// input, so a dispatch omitting it is rejected.
    pub fn requires_ship_id(mut self) -> Self {
        self.env.insert("STUB_REQUIRES_SHIP_ID".into(), "1".into());
        self
    }
...
case "$1 ${2:-}" in
  ...
  "workflow run")
    # Dispatch returns nothing, exactly like the real thing. Capture the ref
    # so `run list` only reports a new run on the ref actually dispatched —
    # which is how gh-ship correlates now, and how a wrong ref is simulated.
    #
    # STUB_REQUIRES_SHIP_ID=1 emulates a workflow generated by an older
    # gh-ship: a dispatch omitting `ship_id` is rejected, exactly as GitHub
    # rejects one missing a required input.
    prev=""
    HAS_SHIP_ID=0
    for arg in "$@"; do
      case "$arg" in
        ship_id=*) HAS_SHIP_ID=1 ;;
      esac
      if [ "$prev" = "--ref" ]; then
        DISPATCH_REF="$arg"
      fi
    ...
    if [ "${STUB_REQUIRES_SHIP_ID:-0}" = "1" ] && [ "$HAS_SHIP_ID" = "0" ]; then
      echo "required input 'ship_id' not provided" >&2
      exit 1
    fi
    echo "${DISPATCH_REF:-}" > "${STUB_LOG:-/tmp/stub}.dispatchref"
    ;;
  ...
  "run list")
    ...
    # A run of the same workflow triggered by something other than a
    # dispatch — creating the staging branch is a push. gh-ship must ignore
    # it, so it is listed alongside whatever else is visible.
    NOISE=""
    if [ "${STUB_PUSH_RUN:-0}" = "1" ]; then
      NOISE="$(printf '{"databaseId":99,"displayTitle":"prepare-release","status":"completed","conclusion":"success","url":"https://github.com/%s/actions/runs/99","headBranch":"%s","event":"push"}' "$REPO" "$BRANCH")"
    fi
    RUNS=""
    if [ -n "$REF" ] && [ "$REF" = "$BRANCH" ]; then
      # The run this test dispatched, on the ref it was dispatched on.
      RUNS="$(printf '{"databaseId":42,"displayTitle":"prepare-release","status":"%s","conclusion":"%s","url":"https://github.com/%s/actions/runs/42","headBranch":"%s","event":"workflow_dispatch"}' \
        "${STUB_RUN_STATUS:-completed}" "${STUB_RUN_CONCLUSION:-success}" "$REPO" "$BRANCH")"
      ...
      RUNS="$(printf '{"databaseId":41,"displayTitle":"publish-release","status":"%s","conclusion":"%s","url":"https://github.com/%s/actions/runs/41","headBranch":"%s","event":"workflow_dispatch"}' \
        "${STUB_EXISTING_RUN_STATUS:-completed}" "${STUB_EXISTING_RUN_CONCLUSION:-success}" "$REPO" "$BRANCH")"
    fi
    if [ -n "$RUNS" ] && [ -n "$NOISE" ]; then
      printf '[%s,%s]\n' "$RUNS" "$NOISE"
    else
      printf '[%s]\n' "${RUNS}${NOISE}"
    fi
  ...
  "run view")
    printf '{"databaseId":%s,"displayTitle":"prepare-release","status":"%s","conclusion":"%s","url":"https://github.com/%s/actions/runs/%s","headBranch":"release/next","event":"workflow_dispatch"}\n' \
      "${3:-42}" "${STUB_RUN_STATUS:-completed}" "${STUB_RUN_CONCLUSION:-success}" "$REPO" "${3:-42}"
    ;;
